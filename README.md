...
它不是通用聊天机器人，也不是已交付的园区生产系统；无 GPU 也能一条命令复现核心结论。
...
没有 GPU 时用 `docker compose -f docker-compose.demo.yml up -d --build` 配合 `scripts/demo-compatibility`，储存禁忌直接走确定性规则引擎出结论（验收细节见脚本内注释）。
...
- **2026-09-17** — 演示视频入口切换到 B站（BV1ieeu66E1A），README 与文档口径统一。
...
给 MIT 开源署名 + 完整架构经验 + 从验证场走向生产系统的开源协作经历。感兴趣直接开 Issue / PR，或 Gitee 私信联系负责人刘超越。
