...
仓库根目录只放 README 与代码；跨模块规范都在 `docs/platform/`。
...
| 模块 | 代码 | 说明书 | 负责 |
|------|------|--------|------|
| platform（跨模块） | （无独立工程） | 见下表「跨模块规范」 | 工程治理、血谱、测试总纲、等保、CHANGELOG |
...
| e2e | `agent1-web/e2e*` | [e2e/说明书.md](e2e/说明书.md) | Playwright 三档：Mock / 真 API / 演示录片 |
...
| 仓库首页、怎么启动 | 根目录 [README.md](../README.md) | 无 GPU 走 demo compose |
...
原始 mp4 仍留在 Gitee Release `os2026-demo`，不进 Git。
...
`_archive/project/` 里还有 6 月以前的全景/差距报告，属内部底稿，不当入口。

---

## 项目介绍在哪

- 项目是什么、怎么跑：根目录 [README.md](../README.md)
- 各模块职责与契约：本文件「按模块」表里的说明书
- 部署与 GPU 实测：[infra/说明书.md](infra/说明书.md)
