...
### 2026-09-20 — 提交材料移出仓库 + 文档措辞中性化

- `docs/platform/` 下 `os2026-*` 系列提交材料（md/html/pdf、渲染脚本、配图）移出公开仓（保留在仓库外 `contest-submit/` 目录，PDF 仅作邮件附件）。`README.md`、`docs/README.md` 相关引用同步清理。
- 近两日新增文案中的场景化措辞按中性口径替换（详见内部脱敏字典）。未改代码、规则引擎、种子、CI。

### 2026-09-20 — README 首页重构

- 根 `README.md`：标题改公开作品名，新增项目简介（链接表+技术栈）、项目结构目录树、全文锚点目录；启动只保留 GPU 主路径（无 GPU 一句话指向 demo compose）；架构节收敛为双通道+检索算法；介绍/进展/同行/愿景升为一级节；边界与文档节压缩。口径与《目录地图》《运行时端口登记》一致，未夸大交付状态。

### 2026-09-17 — 演示视频入口切换到 B站

- 演示视频正式入口改为 B站：`https://www.bilibili.com/video/BV1ieeu66E1A`。同源更新：根 `README.md`、`docs/README.md`、`release-notes-v0.1.0.md`。
- 原始 mp4 仍保留在 Gitee Release `os2026-demo`，不进 Git（`docs/e2e/说明书.md` 口径不变）。未改代码、规则引擎、种子、CI。
