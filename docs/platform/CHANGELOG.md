...
### 2026-09-21 — README 简介扩写并精简结构

- 根 `README.md`：项目简介扩写为「现场问题 → 结构化结论 → 可追溯」叙事，补充结论回溯与审计哈希链、无 GPU 仍出结论、一条命令复现示例（甲醇 × 硝酸）；删除简介内的双通道拆分细述、整节「目录」锚点列表、整节「边界」。口径与工程治理一致，未改代码、规则引擎、种子、CI。

### 2026-09-20 — README 首页去比赛化

- 根 `README.md` 回归开源项目门面：评委/大赛/参赛/os2026 字样中性化或移除；链接表「大赛作品介绍」改为「设计文档」（血谱/架构导向图），演示视频标注（B站）。
- `docs/platform/os2026-作品介绍.*` 原位保留不删。未改代码、规则引擎、种子、CI。

### 2026-09-20 — README 重构为大赛门面版

- 根 `README.md`：标题改公开作品名，新增项目简介（链接表+技术栈）、项目结构目录树、全文锚点目录；启动只保留 GPU 主路径（无 GPU 一句话指向 demo compose 与《作品介绍》附录 A）；架构节收敛为双通道+检索算法；介绍/进展/同行/愿景升为一级节（招募措辞不出现「招人」）；边界与文档节压缩。口径与《作品介绍》《目录地图》《运行时端口登记》一致，未夸大交付状态。
