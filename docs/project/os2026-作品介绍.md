...
对应评分关注点：双通道（技术创新）；无 GPU 可复现（场景落地）；MIT、NOTICE、不含国标全文（开源治理）；个人维护加三条规划（长期发展）。本文讲为什么、怎么复现、边界在哪；演示视频先给出甲醇与硝酸禁配及 GB 15603，再按侧栏走一遍已有界面。工程目录与仓库仍用 Agent1 / agent-system。评委若只读摘要，应能带走三句话：事实不由模型生成；无 GPU 也能验收；仓库不含国标全文、也不是等保测评对象。
...
前端主页面：登录 `/login`、储存兼容性 `/storage/compatibility`、合规检查 `/compliance`、审计日志 `/audit`（当前路由要求 admin）、仪表盘 `/dashboard`。演示视频会按侧栏把已有界面走一遍，但不要把全部页面写成已闭环交付的生产模块。其余能力各用一句话概括：合规历史、巡检计划 / 轮次 / 报告、工单、资产台账、危化品查询、AI 合规助手、监管核查辅助、应急页、知识图谱页。其中应急与知识图谱界面与接口已存在，深度能力仍在演进，不作生产承诺。
...
演示视频与官网报名表一致，提交时填写。PDF 不替代视频。开场应先看到禁配结论与 GB 15603，再按侧栏走已有功能；不要把后半段界面写成已闭环交付。
...
第三方摘要见 `NOTICE`，直接依赖清单见 `THIRD_PARTY_NOTICES.md`。后端主要依赖许可：.NET / ASP.NET Core / Semantic Kernel 为 MIT；PostgreSQL / Npgsql 为 PostgreSQL License；Dapper 为 Apache-2.0；Vue 3 等前端为 MIT。必须点名的 copyleft：NuGet 包 `PDFtoImage` 为 AGPL-3.0-or-later，使用方需自行评估，本文不隐瞒该许可。llama.cpp 为 MIT，二进制不随本仓分发。NOTICE 再次声明不含国标全文、非等保测评对象。
...
演示视频与官网报名表一致，提交时填写。分镜：片头定位与仓库 URL；登录一次；按侧栏走已有界面；主场景甲醇 × 硝酸储存兼容性输出 GB 15603；审计哈希链；片尾开源复现。应急与知识图谱只展示已有界面，不作生产承诺。
