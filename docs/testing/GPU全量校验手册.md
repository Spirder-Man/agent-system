...
6. L4：`/api/Eval/run` → `completed`；识图 200 且 ≥3s；63 条指标对照 [测试总纲 §4.2](./测试总纲.md) 历史基线（退化写入 `alerts[]`，**不**因未达理想 90%/85% 否决栈）  
...
> 分层与历史指标见 [测试总纲.md](./测试总纲.md) v2.2（[蓝图](./测试总纲-蓝图.html)）§4.2；日志六维见 [系统日志解读与排障实战指南](./系统日志解读与排障实战指南.md) / [系统日志阅读与分析实战教学](./系统日志阅读与分析实战教学.md)。  
> Real E2E 失败模式：[E2E测试关注点-蓝图.html](./E2E测试关注点-蓝图.html)。
